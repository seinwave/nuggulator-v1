...
class Results extends Component {
  render(props) {
    ...
    if (denominations.length < 1) {
      ...
      denominationsText = `only ${denominations[0]}-packs available in your origin country`;
    } else if (denominations.length === 2) {
      denominationsText = `only ${denominations[0]}- and ${denominations[1]}-packs available in your origin country`;
    } else if (denominations.length > 2) {
      for (let i = 0; i <= denominations.length - 2; i++) {
        denominationsText.push(` ${denominations[i]}-`);
        denominationsText.join(" , ");
      }
      denominationsText.push(
        ` and ${
          denominations[denominations.length - 1]
        }-packs available in your origin country`
      );
    }
    ...
    for (let i = 0; i < Object.keys(counted).length; i++) {
      countedText.push(
        `You need ${Object.values(counted)[i]} pack(s) of ${
          Object.keys(counted)[i]
        }.`
      );
    ...
    return (
      <div>
        <div className="center pa4 br3 shadow-5" id="results">
          <header>
            <h1>Nuggulation Results</h1>
            <p className="f3">{`You're travelling ${prettyDistance}.`}</p>
            <img
              alt="Result map"
              src={`https://maps.googleapis.com/maps/api/staticmap?size=500x300&path=color:0x0000ff|weight:5|enc:${polyline}&key=AIzaSyAKS01fQkbYZJo2M-OssfMxdpGoF7c7EhY`}
            />
        ...
        <div>
          <button
            className="w-30 grow pa4 mv5 f2 pointer ph3 pv2 dib white bg-gray"
            onClick={() => onRouteChange("start")}
          >
            {" "}
            New trip?
          </button>
